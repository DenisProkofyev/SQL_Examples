...
ALTER TABLE bands
RENAME COLUMN name TO Names_of_all_the_Bands;

ALTER TABLE bands
RENAME COLUMN Names_of_all_the_Bands TO name;
...
INSERT INTO albums(name,release_year,band_id) VALUES ('Sehnsucht',1997,8);
...
SELECT bands.name AS 'Band', COUNT(songs.name) AS 'Number of Songs' FROM bands
JOIN albums on bands.id = albums.band_id
JOIN songs on albums.id = songs.album_id
GROUP BY albums.band_id
